...
# VECTOR_BACKEND fixture — Pinecone is now the only backend (chroma removed)
# No direct chromadb / Chroma collection imports — all vector access is mocked
# via _query_pinecone / _query_vector_store; fixture clears Pinecone cache.
@pytest.fixture(params=["pinecone"])
def vector_backend(request, monkeypatch):
    backend = request.param
    monkeypatch.setenv("VECTOR_BACKEND", backend)
    # Clear cached Pinecone index so backend switch is respected (chroma cache removed)
    import agent.nodes.retrieve as ret
    ret._pinecone_index = None
    yield backend
    # Teardown: clear env and caches
    monkeypatch.delenv("VECTOR_BACKEND", raising=False)
    ret._pinecone_index = None

# Helper to make fake docs (same shape as _query_chroma / _query_pinecone)
def fake_docs(sims):
    return [
...
def test_retrieval_returns_docs_above_threshold_no_fallback(vector_backend):
    # sim 0.8 > 0.35 => grounded, no fallback
    with patch("agent.nodes.retrieve._query_pinecone", return_value=fake_docs([0.8, 0.7])):
        with patch("agent.nodes.retrieve._query_vector_store", return_value=fake_docs([0.8, 0.7])):
...
def test_retrieval_low_similarity_triggers_fallback(vector_backend):
    ...
    with patch("agent.nodes.retrieve._query_pinecone", return_value=fake_docs([0.2, 0.1])):
        with patch("agent.nodes.retrieve._query_vector_store", return_value=fake_docs([0.2, 0.1])):
...
def test_retrieval_empty_chroma_triggers_fallback(vector_backend):
    live = [{"content": "bis hallmarking", "source_url": "https://bis.gov.in/hall", "source_title": "Hallmarking", "chunk_id": ""}]
    with patch("agent.nodes.retrieve._query_pinecone", return_value=[]):
        with patch("agent.nodes.retrieve._query_vector_store", return_value=[]):
...
def test_retrieval_multi_topic_merge_contains_all_topics(vector_backend):
    # Mock per-topic retrieval to simulate decomposition
    def fake_vector_store(query, k=5):
        if "ISI" in query:
    ...
    with patch("agent.nodes.retrieve._query_pinecone", side_effect=fake_vector_store):
        with patch("agent.nodes.retrieve._query_vector_store", side_effect=fake_vector_store):
...
def test_retrieval_single_topic_not_polluted(vector_backend):
    # Ensure single-topic query doesn't get cross-topic noise from decomposition
    def fake_vector_store_single(query, k=5):
    ...
    with patch("agent.nodes.retrieve._query_pinecone", side_effect=fake_vector_store_single):
        with patch("agent.nodes.retrieve._query_vector_store", side_effect=fake_vector_store_single):
